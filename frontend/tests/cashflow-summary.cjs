...
;(async () => {
  ...
  assert.equal(vm.runInContext('reportSummary.value[4].label', context), 'Cash and Bank Balance')
  assert.match(vm.runInContext('reportSummary.value[3].description', context), /2026-09-22/)
  assert.deepEqual(calls[0], ['1000-01-01', '2026-09-22', 'Company', true])
  assert.deepEqual(calls[1], ['1000-01-01', fixedToday, 'Company', true])
  assert.match(vm.runInContext('reportSummary.value[4].description', context), /2026-09-30/)
