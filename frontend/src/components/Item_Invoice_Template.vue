          ...
          <table class="w-full text-sm border-collapse border-l border-t border-[var(--color-border)]">
            <thead>
              <tr class="sticky top-0 z-10 bg-[var(--color-lowlight)] border-b border-[var(--color-border)]">
                ...
                <th v-if="doctype === 'Purchase Order'" class="border-r border-b border-[var(--color-border)] px-1.5 py-2 text-right text-4xl font-normal uppercase tracking-wider text-[var(--color-text)] w-24">Max Qty</th>
              ...
              <template v-for="(item, idx) in items" :key="idx">
                <slot name="row" :item="item" :index="idx" :formatQty="formatQty" :format="format">
                   <tr 
                    ...
                    tabindex="0"
                    @keydown.delete.prevent="$emit('delete-item', idx)"
