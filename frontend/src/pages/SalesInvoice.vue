        ...
        <div class="flex flex-col h-full overflow-hidden">
          <div class="flex-1 overflow-y-auto px-4 pb-4 pt-2 scrollbar-none">
            <template v-if="isSubmitted">
              ...
              <div v-else class="max-h-[160px] overflow-y-auto mb-4 custom-scrollbar">
                <table class="w-full text-left text-2xl border-collapse">
                  <thead class="sticky top-0 bg-[var(--color-bg)] z-10">
                    <tr class="text-[var(--color-text-muted)] border-b border-[var(--color-border)]/50">
                      <th class="py-1 pr-1 font-bold">Date</th>
                      <th class="py-1 px-1 font-bold">Account</th>
                      <th class="py-1 pl-1 text-right font-bold">Amount</th>
                    </tr>
                  </thead>
                  <tbody class="divide-y divide-[var(--color-border)]/30">
                    <tr v-for="p in linkedPayments" :key="p.name" class="text-[var(--color-text)]">
                      <td class="py-1.5 pr-1 font-mono leading-none whitespace-nowrap">{{ formatDateShort(p.posting_date) }}</td>
                      <td class="py-1.5 px-1 leading-none whitespace-nowrap">
                        {{ p.account ? p.account.split(' - ')[0] : '-' }}
                      </td>
                      <td class="py-1.5 pl-1 text-right font-mono leading-none font-bold text-[var(--color-success)]">{{ format(p.amount) }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
...
async function handleSave() {
  ...
  if (loading !== 0) {
    ...
    if (acct) additionalCharges.push({ charge_type: 'Actual', account_head: acct, tax_amount: loading, description: 'Tax Collected' })
  }
  if (packing !== 0) {
...
function handleExport() {
  ...
  a.href = url
  a.download = `${invoiceNo.value || 'sales-invoice'}-items.csv`
  a.click()
...
function detectPriceChange(item, focusTarget) {
  ...
  const effectiveRate = currentDiscount > 0.001
    ? parseFloat((standardRate * (1 - getDisc3p(currentDiscount) / 100)).toFixed(precision))
    : currentRate
...
watch(ignoreModifier, () => {
  ...
  items.value.forEach(item => {
    ...
    item._base_rate = base
    item.rate = parseFloat(((base || 0) * combinedFactor(item.item_code)).toFixed(precision))
    item.amount = parseFloat(((item.qty || 0) * item.rate * (1 - getDisc3p(item.discount) / 100)).toFixed(precision))
  })
})
...
watch(halfTaxDiscount, (enabled) => {
  ...
  items.value = items.value.map(item => {
    ...
    if (enabled) {
      ...
      const disc = t > 0 ? parseFloat(((t / (2 * (100 + t))) * 100).toFixed(3)) : 0
