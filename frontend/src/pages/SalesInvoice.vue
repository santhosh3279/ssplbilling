    ...
    >
      ...
      <template #row="{ item, index }">
        ...
        >
          ...
          <td class="px-2 py-1 border-r border-[var(--color-border)] text-2xl font-medium" :class="selectedRowIdx === index && !item.deleted && !item._is_free ? 'text-[var(--color-text)]' : 'text-[var(--color-text)]'">
            {{ item.item_name }}
          ...
          <td class="p-0 border-r border-[var(--color-border)]">
            <input v-if="editingRowIdx === index && editingField === 'disc'"
              ...
              @keydown.escape="exitEditMode(index)"
            />
            <span v-else class="block px-2 py-1 text-2xl font-mono text-right" :class="selectedRowIdx === index && !item.deleted ? 'text-[var(--color-text)]' : 'text-[var(--color-warning)]'">{{ item.discount || '0' }}</span>
          </td>
