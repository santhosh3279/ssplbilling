      ...
      <div class="flex flex-wrap items-end gap-6">
        ...
        <div class="flex flex-col gap-1">
          ...
          <input
            v-model="form.date"
            type="date"
            class="rounded border border-slate-600 bg-slate-800 px-2 py-1.5 text-sm font-bold text-slate-200 outline-none focus:border-blue-500 tabular-nums"
          />
        </div>

        <!-- Time -->
        <div class="flex flex-col gap-1">
          <label class="text-[10px] font-bold uppercase text-slate-400">Time</label>
          <input
            v-model="form.time"
            type="time"
            step="1"
        ...
        <div class="flex flex-col gap-1 relative" ref="customerWrap">
          <label class="text-[10px] font-bold uppercase text-slate-400">Customer</label>
          <input
            ref="customerInput"
            v-model="customerQuery"
            type="text"
            placeholder="Search customer..."
            class="w-56 rounded border border-slate-600 bg-slate-800 px-2 py-1.5 text-sm text-slate-200 outline-none focus:border-blue-500"
            autocomplete="off"
            ...
            <table class="w-full text-sm border-collapse">
              <thead>
                <tr class="sticky top-0 z-10 bg-slate-800 border-b border-slate-700">
                  <th class="w-10 px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-slate-400">#</th>
                  <th class="w-48 px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-slate-400">Item</th>
                  <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-slate-400">Item Name</th>
                  <th class="w-28 px-4 py-3 text-right text-xs font-bold uppercase tracking-wider text-slate-400">Qty</th>
                  <th class="w-32 px-4 py-3 text-right text-xs font-bold uppercase tracking-wider text-slate-400">Rate</th>
                  <th class="w-36 px-4 py-3 text-right text-xs font-bold uppercase tracking-wider text-slate-400">Amount</th>
              ...
              <tbody>
                ...
                <tr
                  ...
                  tabindex="-1"
                  class="group border-b border-slate-700 last:border-0 outline-none transition-colors"
                  :class="selectedRow === idx ? 'bg-blue-900/30' : 'hover:bg-slate-800/40'"
                  @click="selectRow(idx)"
                ...
                >
                  ...
                  <td class="px-4 py-2 relative">
                    <div v-if="selectedRow === idx" class="relative">
                      <input
                        ...
                        v-model="row.item"
                        class="w-full rounded border border-slate-600 bg-slate-800 px-2 py-1 font-mono text-slate-200 outline-none focus:border-blue-500"
                      ...
                      >
                        <div
                          ...
                          :class="i === rowItemHighlight ? 'bg-blue-600 text-white' : 'text-slate-200 hover:bg-slate-800'"
                          @mousedown.prevent="pickRowItem(idx, it)"
                          @mouseover="rowItemHighlight = i"
                        >
                          <div class="font-mono font-semibold">{{ it.item_code }}</div>
                          <div class="text-[10px]" :class="i === rowItemHighlight ? 'text-blue-200' : 'text-slate-500'">{{ it.item_name }}</div>
                        </div>
                      </div>
                    </div>
                    <span v-else class="font-mono font-semibold text-blue-400">{{ row.item || '--' }}</span>
                  </td>
                  ...
                  <td class="px-4 py-2">
                    <input
                      ...
                      class="w-full rounded border border-slate-600 bg-slate-800 px-2 py-1 text-slate-200 outline-none focus:border-blue-500"
                      @keydown.enter.prevent="focusField('qty', idx)"
                      @keydown.tab.prevent="focusField('qty', idx)"
                      @keydown.down.prevent="moveRow(idx, 1)"
                      @keydown.up.prevent="moveRow(idx, -1)"
                    />
                    <span v-else class="text-slate-200">{{ row.item_name || '--' }}</span>
                  </td>
                  ...
                  <td class="px-4 py-2 text-right font-mono">
                    <input
                      ...
                      min="0"
                      step="any"
                      class="w-full rounded border border-transparent bg-transparent px-2 py-1 text-right text-slate-200 focus:border-blue-400 focus:bg-slate-800 outline-none"
                      ...
                      @keydown.up.prevent="moveRow(idx, -1)"
                    />
                    <span v-else class="text-slate-300">{{ row.qty }}</span>
                  </td>
                  ...
                  <td class="px-4 py-2 text-right font-mono">
                    <input
                      ...
                      @keydown.up.prevent="moveRow(idx, -1)"
                    />
                    <span v-else class="text-slate-300">{{ (row.rate || 0).toFixed(2) }}</span>
                  </td>

                  <!-- Amount -->
                  <td class="px-4 py-2 text-right font-mono font-bold text-slate-100">{{ (row.amount || 0).toFixed(2) }}</td>

                  <!-- Delete -->
                  <td class="px-4 py-2 text-center">
                    <button
                      class="rounded p-1 text-slate-600 hover:bg-red-900/20 hover:text-red-400 opacity-0 group-hover:opacity-100 transition"
                ...
                <!-- NEW ROW -->
                <tr class="border-b border-slate-700 bg-blue-900/10" :class="{ 'bg-blue-900/30 ring-2 ring-inset ring-blue-500': selectedRow === -1 }">
                  ...
                  <td class="px-4 py-3 relative">
                    <input
                      ...
                      class="w-full rounded border border-slate-600 bg-slate-800 px-2 py-1 font-mono text-slate-200 outline-none focus:border-blue-500 shadow-sm"
                    ...
                    >
                      <div
                        ...
                        :key="it.item_code"
                        class="cursor-pointer px-4 py-2 text-sm"
                        :class="i === newItemHighlight ? 'bg-blue-600 text-white' : 'text-slate-200 hover:bg-slate-800'"
                        @mousedown.prevent="pickNewItem(it)"
                        @mouseover="newItemHighlight = i"
                      >
                        <div class="font-mono font-semibold">{{ it.item_code }}</div>
                        <div class="text-[10px]" :class="i === newItemHighlight ? 'text-blue-200' : 'text-slate-500'">{{ it.item_name }}</div>
                      </div>
                    </div>
                  </td>
                  <td class="px-4 py-3">
                    <div class="text-slate-500 italic text-sm">{{ newItem.item_name || 'Item name...' }}</div>
                  </td>
                  <td class="px-4 py-3 text-right">
                    <input
                      ...
                      min="0"
                      step="any"
                      class="w-24 rounded border border-slate-600 bg-slate-800 px-2 py-1 text-right font-mono text-slate-200 outline-none focus:border-blue-500 shadow-sm"
                  ...
                  <td class="px-4 py-3 text-right">
                    <input
                      ...
                      min="0"
                      step="0.01"
                      class="w-28 rounded border border-slate-600 bg-slate-800 px-2 py-1 text-right font-mono text-slate-200 outline-none focus:border-blue-500 shadow-sm"
                      @keydown.enter.prevent="addNewRow"
                    />
                  </td>
                  <td class="px-4 py-3 text-right font-mono text-slate-500 font-bold">{{ ((newItem.qty || 0) * (newItem.rate || 0)).toFixed(2) }}</td>
