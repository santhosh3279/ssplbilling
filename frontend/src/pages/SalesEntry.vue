        ...
        <div class="flex flex-[7] flex-col overflow-hidden">
          <div class="flex-1 overflow-y-auto">
            <table class="w-full text-sm border-collapse border-l border-t border-slate-700">
              ...
              <tbody>
                <tr v-for="(item, idx) in items" :key="idx" :ref="el => setRowRef(el, idx)" tabindex="-1" class="cursor-pointer border-b border-slate-700 outline-none transition-colors" :class="{ 'bg-blue-900/30 border-l-2 border-l-blue-500': selectedRow === idx && !item.deleted && !item._is_free, 'bg-green-900/10 border-l-2 border-l-green-600': item._is_free, 'bg-green-900/5 border-l-2 border-l-green-700': !item._is_free && item._rule_discount != null && !item.deleted, 'bg-red-900/10': item.deleted, 'hover:bg-slate-800/50': !item.deleted && !item._is_free && item._rule_discount == null && selectedRow !== idx }" :style="{ fontSize: dynamicRowStyle.fontSize }" @click="selectRow(idx)" @keydown="onRowKeydown($event, idx)">
              ...
              <div class="flex flex-col gap-2">
                ...
                <div class="flex items-center justify-between text-lg">
                  <div class="flex items-center gap-1.5">
                    ...
                    <input ref="freightInput" type="number" v-model.number="freightAmt" :disabled="billDocStatus !== 0 || billSaved" min="0" step="1" class="w-24 rounded border border-slate-600 bg-slate-800 px-1.5 py-1 text-right text-lg font-bold text-slate-100 outline-none focus:border-blue-500 disabled:bg-slate-900" @keydown.enter="saveButton?.focus()" />
                  </div>
