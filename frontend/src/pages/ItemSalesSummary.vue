    ...
    <header class="sticky top-0 z-40 border-b border-[var(--color-border)] bg-[var(--color-surface)] px-6 py-3">
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h1 class="text-lg font-bold text-[var(--color-text)] uppercase tracking-wider">Item Sales Summary</h1>
            <p class="text-xs text-[var(--color-text-muted)]">Consolidated sales by item code and series</p>
          </div>
        </div>
    ...
    <main class="flex-1 overflow-auto p-6">
      <div v-if="loading" class="flex h-64 items-center justify-center">
        <div class="flex flex-col items-center gap-3">
          ...
          <p class="text-sm text-[var(--color-text-muted)] font-medium">Fetching sales summary...</p>
        </div>
      </div>
      ...
      <div v-else class="w-full">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 max-w-5xl mx-auto">
          ...
          <div class="bg-[var(--color-surface)] border border-[var(--color-border)] p-6 rounded-2xl shadow-sm">
            <p class="text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-widest mb-1">Unique Items</p>
            <p class="text-3xl font-black text-[var(--color-info)]">{{ filteredData.length }}</p>
          </div>
        </div>
...
onMounted(async () => {
  ...
  try {
    ...
    if (seriesList.value.length > 0) {
      ...
      error.value = 'No invoice series allowed for this user.'
    }
  } catch (e) {
    error.value = e.message || 'Failed to initialize allowed naming series.'
  } finally {
    loading.value = false
  }
})
...
async function fetchData() {
  ...
  error.value = ''
  try {
    const data = await getItemSummaryReport(selectedSeries.value, fromDate.value, toDate.value)
    reportData.value = data || []
  } catch (e) {
    error.value = e.message || 'Failed to fetch item summary report data'
  } finally {
    loading.value = false
  }
}
...
function exportToExcel() {
  ...
  const headers = ['S.No', 'Item Code', 'Item Name', 'UOM', 'Total Quantity', 'Total Taxable Value']
  ...
  utils.book_append_sheet(wb, ws, 'Item Sales Summary')

  const series = selectedSeries.value.replace(/[^A-Za-z0-9]/g, '')
  writeFile(wb, `ItemSalesSummary_${series}_${fromDate.value}_to_${toDate.value}.xlsx`)
}
</script>
