<template>
  <div class="flex min-h-screen flex-col bg-[var(--color-bg)]">
    ...
    <div class="flex flex-1 overflow-hidden">
      <!-- Sidebar with Buttons (Reorganized into Four Columns) -->
      <aside class="w-[74rem] border-r border-[var(--color-border)] bg-[var(--color-surface)]/30 p-6 overflow-y-auto">
        <div class="grid grid-cols-4 gap-6 h-full align-top">
          ...
          <!-- Column 4: Accounts -->
          <div class="flex flex-col gap-3">
            <h3 class="text-sm font-semibold uppercase tracking-wider text-[var(--color-text-muted)] border-b border-[var(--color-border)] pb-2 mb-2">Accounts</h3>
      ...
      <!-- Body placeholder or Stock Report -->
      <main class="flex-1 flex flex-col overflow-hidden bg-[var(--color-bg)]">
        <!-- Stock Report View -->
        <div v-if="isStockReportActive" class="flex flex-col flex-1 overflow-hidden p-6">
          <div class="flex flex-wrap items-center justify-between gap-4 border-b border-[var(--color-border)] pb-4 mb-4">
            <div>
              <h2 class="text-2xl font-bold text-[var(--color-text)]">Stock Status Report</h2>
              <p class="text-base text-[var(--color-text-muted)]">Real-time stock status with advanced filters</p>
            </div>
            
            <button
              @click="exportStockReportToExcel"
              :disabled="loadingStockData || !stockReportRows.length"
              class="flex items-center gap-2 rounded-lg bg-[var(--color-success)] px-4 py-2 text-base font-semibold text-white hover:bg-[var(--color-success)]/80 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
            >
              📥 Export to Excel
            </button>
          </div>
          ...
          <!-- Report Data Table -->
          <div class="flex-1 overflow-auto rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)]">
            <div v-if="loadingStockData" class="flex h-64 items-center justify-center">
              <span class="animate-spin text-3xl">⏳</span>
              <span class="ml-2 text-sm text-[var(--color-text-muted)]">Loading stock status...</span>
            </div>
            ...
            <table v-else class="w-full border-collapse text-left text-sm">
              <thead class="sticky top-0 bg-[var(--color-lowlight)] border-b border-[var(--color-border)] z-10 text-[var(--color-text)]">
                <tr>
                  <th class="px-4 py-2.5 font-semibold">Item Code</th>
                  <th class="px-4 py-2.5 font-semibold">Item Name</th>
                  <th class="px-4 py-2.5 font-semibold">Warehouse</th>
                  <th class="px-4 py-2.5 font-semibold text-right">Actual Stock</th>
                  <th class="px-4 py-2.5 font-semibold text-right">Safety Stock</th>
                  <th class="px-4 py-2.5 font-semibold text-right">Max Stock</th>
                  <th class="px-4 py-2.5 font-semibold">UOM</th>
                  <th class="px-4 py-2.5 font-semibold">Linked Suppliers</th>
                </tr>
              </thead>
...
import { getSalesTaxRegister, getQuotationTaxRegister, getQuotationSeries, getHsnSummaryReport, getQuotationHsnSummaryReport, getItemSummaryReport, getStoreWiseItemSalesReport, getIncomeAccounts, getCashflowReport, fetchStockReportFilters, fetchStockReportData } from '../api.js'
...
onMounted(async () => {
  ...
  // Load stock status report filters
...
// ── Stock Status Report Logic ────────────────────────────────────────────────
const isStockReportActive = ref(false)
...
function selectStockReport() {
  isStockReportActive.value = true
  loadStockReportData()
}
