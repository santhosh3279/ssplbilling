...
const indirectExpenseEntries = ref([])
...
async function fetchData() {
  ...
  try {
    ...
    indirectExpenseEntries.value = res.indirect_expense_entries || []
...
function exportToExcel() {
  ...
  // Add Indirect Expense Particulars Sheet (entry-level details)
  if (indirectExpenseEntries.value.length) {
    const detHeaders = ['S.No', 'Cost Center', 'Date', 'Voucher Type', 'Voucher No', 'Expense Head', 'Particulars', 'Amount']
    ...
    const detTotal = indirectExpenseEntries.value.reduce((sum, e) => sum + (e.amount || 0), 0)
    detRows.push(['', 'GRAND TOTAL', '', '', '', '', '', Math.round(detTotal)])
    ...
    utils.book_append_sheet(wb, wsDet, 'Indirect Expense Particulars')
  }
