      ...
      <div class="flex flex-1 flex-col overflow-hidden bg-slate-900">
        ...
        <div class="flex items-center justify-between border-b border-slate-700/60 bg-slate-800/60 px-4 py-1">
          <div class="flex items-center gap-3">
            ...
            <span class="text-xs font-semibold text-slate-300">Purchase Entry</span>
          </div>
        ...
        <div class="flex-1 flex items-center gap-4 border-l border-slate-700 pl-6 overflow-hidden">
          ...
          <div class="flex items-baseline gap-4 min-w-0">
            ...
            <div v-if="selectedSupplierDetails" class="flex items-center gap-3 min-w-0">
              ...
              <span v-if="selectedSupplierDetails.mobile_no" class="whitespace-nowrap text-xl text-slate-500 leading-none">
          ...
          <div v-if="selectedSupplierDetails" class="flex items-center gap-6 ml-auto mr-6">
            ...
            <div class="flex flex-col items-end leading-none border-l border-slate-700 pl-6">
              ...
              <span :class="selectedSupplierDetails.balance > 0 ? 'text-green-400' : 'text-red-400'" class="text-xl tabular-nums">
                &#8377;{{ Math.abs(selectedSupplierDetails.balance || 0).toFixed(2) }} <span class="text-[10px]">{{ selectedSupplierDetails.balance > 0 ? 'DR' : 'CR' }}</span>
              </span>
            </div>
          </div>
        </div>
        ...
        <div class="flex flex-[7] flex-col overflow-hidden">
          <div class="flex-1 overflow-y-auto overflow-x-hidden scrollbar-none">
            <table class="w-full text-sm border-collapse border-l border-t border-slate-700">
              ...
              <tbody>
                <tr v-for="(item, idx) in items" :key="idx" :ref="el => setRowRef(el, idx)" tabindex="-1" class="cursor-pointer border-b border-slate-700 outline-none transition-colors" :class="{ 'bg-[#B0E4CC] border-l-2 border-l-blue-500 text-black': selectedRow === idx && !item.deleted && !item._is_free && !item._rule_discount && !item._supplier_pricing, 'bg-green-900/30 border-l-2 border-l-green-400': item._is_free && !item.deleted, 'bg-green-900/20 border-l-2 border-l-green-600': !item._is_free && item._rule_discount != null && !item.deleted, 'bg-purple-900/20 border-l-2 border-l-purple-500': !item._is_free && item._rule_discount == null && item._supplier_pricing && !item.deleted, 'bg-red-900/10': item.deleted, 'hover:bg-slate-800/50': !item.deleted && !item._is_free && item._rule_discount == null && !item._supplier_pricing && selectedRow !== idx }" :style="{ fontSize: dynamicRowStyle.fontSize }" @click="selectRow(idx)" @keydown="onRowKeydown($event, idx)">
                  <td class="px-3 border-r border-slate-700" :style="{ paddingTop: dynamicRowStyle.paddingTop, paddingBottom: dynamicRowStyle.paddingBottom }"><span class="inline-flex h-5 w-5 items-center justify-center rounded-full" :class="item.deleted ? 'bg-red-900/30 text-red-400' : (selectedRow === idx && !item.deleted && !item._is_free && !item._rule_discount && !item._supplier_pricing ? 'bg-slate-700 text-[#B0E4CC]' : 'bg-slate-800 text-slate-400')" :style="{ fontSize: `${(8 * zoomPercent) / 100}px` }">{{ idx + 1 }}</span></td>
                  <td class="p-0 border-r border-slate-700">
                    <input v-if="selectedRow === idx && !item.deleted" :ref="el => setRef(el, 'code', idx)" v-model="item.item_code" :disabled="billDocStatus !== 0 || billSaved || item._is_free" class="w-full rounded border border-slate-600 font-mono outline-none focus:border-blue-500 disabled:bg-slate-900" :class="selectedRow === idx && !item.deleted && !item._is_free && !item._rule_discount && !item._supplier_pricing ? 'bg-[#B0E4CC] text-black' : 'bg-slate-800 text-slate-200'" style="padding:0" :style="{ fontSize: dynamicRowStyle.fontSize }" @focus="onCodeFocus(idx)" @input="onCodeInput(idx)" @keydown.tab.prevent="focusField('qty', idx)" @keydown.right.prevent="openSearch(item.item_code, idx)" @keydown.enter.prevent="onCodeEnter(idx)" @keydown="handleQuickSearchKeydown($event, idx)" />
                    <span v-else class="font-mono" :class="item.deleted ? 'text-slate-600' : (selectedRow === idx && !item.deleted && !item._is_free && !item._rule_discount && !item._supplier_pricing ? 'text-black' : 'text-slate-400')" :style="{ fontSize: dynamicRowStyle.fontSize }">{{ item.item_code }}</span>
                  </td>
                  <td class="px-2 border-r border-slate-700" :style="{ paddingTop: dynamicRowStyle.paddingTop, paddingBottom: dynamicRowStyle.paddingBottom }"><span :class="item.deleted ? 'text-red-900/50 line-through' : (selectedRow === idx && !item.deleted && !item._is_free && !item._rule_discount && !item._supplier_pricing ? 'text-black' : 'text-slate-200')" :style="{ fontSize: dynamicRowStyle.fontSize }">{{ item.item_name || '--' }}</span><span v-if="item._is_free" class="ml-1 rounded bg-green-900/60 px-1 py-0.5 font-bold text-green-400" :style="{ fontSize: `${(8 * zoomPercent) / 100}px` }">FREE</span><span v-else-if="item.deleted" class="ml-1 font-semibold text-red-500" :style="{ fontSize: `${(8 * zoomPercent) / 100}px` }">DELETED</span></td>
                  <td class="px-2 py-0 border-r border-slate-700 text-right">
                    <input v-if="selectedRow === idx && !item.deleted" :ref="el => setRef(el, 'qty', idx)" type="number" v-model.number="item.qty" :disabled="billDocStatus !== 0 || billSaved || item._is_free" min="0" class="w-full rounded border border-transparent text-right font-mono focus:border-blue-500 focus:outline-none disabled:cursor-not-allowed appearance-none" :class="selectedRow === idx && !item.deleted && !item._is_free && !item._rule_discount && !item._supplier_pricing ? 'bg-[#B0E4CC] text-black focus:bg-[#B0E4CC]' : 'bg-transparent text-slate-200 focus:bg-slate-800'" style="padding:0" :style="{ fontSize: dynamicRowStyle.fontSize }" @keydown.enter.prevent="item.qty !== 0 && ((item.uoms||[]).length > 1 ? focusField('uom', idx) : triggerPricePopupForRow(idx))" @keydown.tab.prevent="(item.uoms||[]).length > 1 ? focusField('uom', idx) : triggerPricePopupForRow(idx)" @keydown.shift.tab.prevent="focusField('code', idx)" @keydown.down.prevent="moveRow(idx, 1)" @keydown.up.prevent="moveRow(idx, -1)" />
                    <span v-else class="block text-right font-mono" :class="item.deleted ? 'text-slate-600' : (selectedRow === idx && !item.deleted && !item._is_free && !item._rule_discount && !item._supplier_pricing ? 'text-black' : 'text-slate-300')" :style="{ fontSize: dynamicRowStyle.fontSize }">{{ isReturn ? -item.qty : item.qty }}</span>
                  </td>
                  <td class="p-0 border-r border-slate-700">
                    <select v-if="selectedRow === idx && !item.deleted && (item.uoms || []).length > 1" :ref="el => setRef(el, 'uom', idx)" v-model="item.uom" :disabled="billDocStatus !== 0 || billSaved" class="w-full rounded border border-transparent font-mono outline-none focus:border-blue-500 disabled:cursor-not-allowed appearance-none" :class="selectedRow === idx && !item.deleted && !item._is_free && !item._rule_discount && !item._supplier_pricing ? 'bg-[#B0E4CC] text-black focus:bg-[#B0E4CC]' : 'bg-transparent text-slate-200 focus:bg-slate-800'" style="padding:0" :style="{ fontSize: dynamicRowStyle.fontSize }" @change="onUomChange(idx)" @keydown.enter.prevent="triggerPricePopupForRow(idx)" @keydown.tab.prevent="triggerPricePopupForRow(idx)" @keydown.shift.tab.prevent="focusField('qty', idx)" @keydown.up.stop @keydown.down.stop>
                      <option v-for="u in item.uoms" :key="u.uom" :value="u.uom">{{ u.uom }}</option>
                    </select>
                    <span v-else class="px-2 font-mono" :class="item.deleted ? 'text-slate-600' : (selectedRow === idx && !item.deleted && !item._is_free && !item._rule_discount && !item._supplier_pricing ? 'text-black' : 'text-slate-400')" :style="{ fontSize: dynamicRowStyle.fontSize }">{{ item.uom || '--' }}</span>
                  </td>
                  <td class="px-2 py-0 border-r border-slate-700 text-right">
                    ...
                    <span v-else class="block text-right font-mono" :class="item.deleted ? 'text-slate-600' : (selectedRow === idx && !item.deleted && !item._is_free && !item._rule_discount && !item._supplier_pricing ? 'text-black' : 'text-slate-300')" :style="{ fontSize: dynamicRowStyle.fontSize }">{{ item.rate.toFixed(2) }}</span>
                  </td>
                  <td class="px-2 py-0 border-r border-slate-700 text-right">
                    ...
                    <span v-else class="block text-right font-mono" :class="item.deleted ? 'text-slate-600' : (selectedRow === idx && !item.deleted && !item._is_free && !item._rule_discount && !item._supplier_pricing ? 'text-black' : 'text-slate-300')" :style="{ fontSize: dynamicRowStyle.fontSize }">{{ item.discount || 0 }}</span>
                  </td>
                  <td class="px-2 text-right border-r border-slate-700" :style="{ paddingTop: dynamicRowStyle.paddingTop, paddingBottom: dynamicRowStyle.paddingBottom }">
                    <span class="font-mono" :class="item.deleted ? 'text-slate-600' : (selectedRow === idx && !item.deleted && !item._is_free && !item._rule_discount && !item._supplier_pricing ? 'text-black' : (item.discount ? 'text-amber-400' : 'text-slate-600'))" :style="{ fontSize: dynamicRowStyle.fontSize }">
                      {{ item.discount ? (item.rate * (1 - (item.discount) / 100)).toFixed(2) : '—' }}
                    </span>
                  </td>
                  <td class="px-2 text-right border-r border-slate-700" :style="{ paddingTop: dynamicRowStyle.paddingTop, paddingBottom: dynamicRowStyle.paddingBottom }">
                    <span class="font-mono" :class="item.deleted ? 'text-slate-600' : (selectedRow === idx && !item.deleted && !item._is_free && !item._rule_discount && !item._supplier_pricing ? 'text-black' : 'text-slate-400')" :style="{ fontSize: dynamicRowStyle.fontSize }">{{ isExempted ? 0 : (item.tax_rate != null ? item.tax_rate : defaultTaxRate) }}</span>
                  </td>
                  <td class="px-2 text-right border-r border-slate-700 font-mono" :class="item.deleted ? 'text-slate-600 line-through' : (selectedRow === idx && !item.deleted && !item._is_free && !item._rule_discount && !item._supplier_pricing ? 'text-black font-bold' : 'text-slate-200')" :style="{ paddingTop: dynamicRowStyle.paddingTop, paddingBottom: dynamicRowStyle.paddingBottom, fontSize: dynamicRowStyle.fontSize }">{{ item.deleted ? '' : ((isReturn ? -1 : 1) * item.qty * item.rate * (1 - (item.discount || 0) / 100)).toFixed(2) }}</td>
                ...
                <tr v-if="billDocStatus === 0 && !billSaved" class="border-b border-slate-700" :class="selectedRow === -1 ? 'bg-blue-900/20' : 'bg-slate-800/30'" :style="{ fontSize: dynamicRowStyle.fontSize }">
                  ...
                  <td class="px-0 text-right border-r border-slate-700" :style="{ paddingTop: dynamicRowStyle.paddingTop, paddingBottom: dynamicRowStyle.paddingBottom }"><input ref="newQtyInput" v-model.number="newQty" type="number" min="0" class="w-full rounded border border-slate-600 bg-slate-800 text-right font-mono text-slate-200 outline-none focus:border-blue-500 appearance-none" style="padding:0" :style="{ fontSize: dynamicRowStyle.fontSize }" @keydown.enter.prevent="newQty !== 0 && ((newPending.uoms||[]).length > 1 ? $nextTick(() => newUomSelect?.focus()) : addNewItemAndOpenPricePopup())" @keydown.tab.prevent="newQty !== 0 && ((newPending.uoms||[]).length > 1 ? $nextTick(() => newUomSelect?.focus()) : addNewItemAndOpenPricePopup())" @keydown.shift.tab.prevent="focusNewCode" /></td>                  <td class="p-0 border-r border-slate-700">
                    <select v-if="(newPending.uoms || []).length > 1" ref="newUomSelect" v-model="newPending.uom" class="w-full rounded border border-slate-600 bg-slate-800 font-mono text-slate-200 outline-none focus:border-blue-500 appearance-none" style="padding:0" :style="{ fontSize: dynamicRowStyle.fontSize }" @change="onNewUomChange" @keydown.enter.prevent="addNewItemAndOpenPricePopup" @keydown.tab.prevent="addNewItemAndOpenPricePopup" @keydown.shift.tab.prevent="focusNewQty">
...
<div class="flex flex-col gap-2 p-2">
    ...
    <CustomerSearchModal
      ...
      :allowed-types="isBiller ? ['Supplier', 'Supplier', 'Employee'] : undefined"
    ...
    <Warning
      ...
      @confirm="showExitModifyWarning = false; startNewBill(); openSupplierSearch()"
    />
...
const availablePriceLists = computed(() => {
  ...
  return unique.length ? unique : ['Standard Buying']
})
...
// ==================== DISCOUNT RULES ====================
function makeRowKey() { return Math.random().toString(36).substring(2, 9) }
function applyDiscountRuleForRow(idx) {}
function reapplyAllDiscountRules() {}

// ==================== CUSTOMER PRICING ====================
const supplierPricing = ref({}) // { item_code: discount_percentage }

async function loadSupplierPricing(cust) {
  if (!cust) { supplierPricing.value = {}; return }
  try {
    const data = await frappeGet('ssplbilling.api.customer_pricing_api.get_customer_pricing', { supplier: cust })
    supplierPricing.value = data || {}
    // Re-apply to items already in the list (fetch may arrive after items were added)
    items.value.forEach((_, idx) => applySupplierPricingForRow(idx))
  } catch (e) {
    supplierPricing.value = {}
  }
}

function applySupplierPricingForRow(idx) {
  const item = items.value[idx]
  if (!item || item.deleted || item._is_free || item._rule_discount != null) return
  
  const factor = supplierPricing.value[item.item_code]
...
function onRateBlur(idx) {
  ...
  // Clear any previous supplier pricing for this row if rate was manually edited
  item._supplier_pricing = false
  
  if (!supplier.value || item._rule_discount != null) return
  ...
  // updating the pricelist or saving for supplier if the new rate > 0
...
function triggerPricePopupForRow(idx) {
  ...
  savePricePopup.value = {
    ...
    item_code: item.item_code,
    item_name: item.item_name,
    multiplication_factor: Math.round(factor * 10000) / 10000,
    rate: item.rate,
    uom: item.uom,
  }
...
async function addNewItemAndOpenPricePopup() {
  ...
  items.value.push({
    ...
    _is_free: false,
    _rule_discount: null,
    _supplier_pricing: false,
  })
  applyDiscountRuleForRow(newIdx)
  applySupplierPricingForRow(newIdx)
...
async function onCodeEnter(idx) {
  ...
  if (r) {
    ...
    applySupplierPricingForRow(idx)
...
async function addNewItem() {
  ...
  applySupplierPricingForRow(items.value.length - 1)
...
async function pickItem(item) {
  ...
  if (itemSearchTargetRow !== null) {
    ...
    applySupplierPricingForRow(itemSearchTargetRow)
...
async function loadInvoice(invoiceName) {
  try {
    ...
    // Set selectedSupplierDetails for display
    try {
      selectedSupplierDetails.value = await fetchSupplierDetails(inv.supplier)
    } catch (e) {
...
watch(supplier, async (newVal) => {
  if (!newVal) {
    
    loadSupplierPricing(null)
    return
  }

  // Fetch sales history and supplier pricing in bulk
  /* fetchCustomerSalesHistory(newVal) not applicable for suppliers */
  loadSupplierPricing(newVal)

  if (!selectedSupplierDetails.value) return
  try {
    const stats = await frappeGet('ssplbilling.api.customersearch_api.get_customer_quick_stats', { supplier: newVal })
    if (stats && selectedSupplierDetails.value && selectedSupplierDetails.value.name === newVal) {
...
async function saveBill() {
  ...
  const payload = {
    ...
    price_list: priceList.value || 'Standard Buying',
...
function handleBack() {
  if (activeItems.value.length > 0 && !billSaved.value) {
    ...
    if (route.query.from === 'ledger' && supplier.value) {
      router.push({ path: '/ledger', query: { supplier: supplier.value } })
    } else {
      router.push('/')
    }
  }
...
useShortcuts(purchaseEntryShortcuts({
  ...
  newCustomer: () => { if (!showPrintModal.value) openSupplierSearch() },
  ...
  contextualBack: () => {
    ...
    if (showSupplierSearchModal.value) { closeCustomerSearchModal(); return }
...
onMounted(async () => {
  ...
  window.addEventListener('wb-global-ledger-search', openSupplierSearch);
...
onUnmounted(() => {
  window.removeEventListener('wb-global-ledger-search', openSupplierSearch);
