...
def get_customer_ledger(customer, from_date=None, to_date=None):
    """Return GL Entry rows for a customer."""
    to_date = to_date or frappe.utils.today()
    from_date = from_date or frappe.utils.add_days(to_date, -90)
    customer_name = frappe.db.get_value("Customer", customer, "customer_name") or customer

    opening_rows = frappe.db.sql("SELECT COALESCE(SUM(debit), 0) - COALESCE(SUM(credit), 0) AS balance FROM `tabGL Entry` WHERE party_type = 'Customer' AND party = %s AND is_cancelled = 0 AND posting_date < %s", (customer, from_date), as_dict=True)
    opening_balance = float(opening_rows[0].balance if opening_rows else 0)

    entries_raw = frappe.db.sql("SELECT posting_date, voucher_type, voucher_no, debit, credit, remarks FROM `tabGL Entry` WHERE party_type = 'Customer' AND party = %s AND is_cancelled = 0 AND posting_date >= %s AND posting_date <= %s ORDER BY posting_date ASC, creation ASC", (customer, from_date, to_date), as_dict=True)

    balance = opening_balance
    entries = []
    for row in entries_raw:
        balance += float(row.debit or 0) - float(row.credit or 0)
        entries.append({"date": str(row.posting_date), "voucher_type": row.voucher_type, "voucher_no": row.voucher_no, "debit": float(row.debit or 0), "credit": float(row.credit or 0), "balance": round(balance, 2), "remarks": row.remarks or ""})

    return {"customer": customer, "customer_name": customer_name, "opening_balance": round(opening_balance, 2), "entries": entries}
...
def get_voucher_detail(voucher_type, voucher_no):
    ...
    if voucher_type == "Sales Invoice":
        base["items"] = [{"item_code": r.item_code, "item_name": r.item_name, "qty": float(r.qty), "rate": float(r.rate)} for r in doc.items]
    return base
